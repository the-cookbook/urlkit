...
describe('object', () => {
  ...
  it('validates object defaults against the declared shape at compile time', () => {
    ...
    ).toThrow(expect.objectContaining({ code: 'invalid-descriptor', path: ['role'] }));

    expect(() =>
      compileRuntimeSchema(
        object({ role: string(), tags: array(string()) }).default({ role: 'admin' } as never),
      ),
    ).toThrow(expect.objectContaining({ code: 'invalid-descriptor', path: ['tags'] }));
  });

  it('validates nested schema defaults when the object is compiled', () => {
    expect(() => compileRuntimeSchema(object({ role: string().default(1 as never) }))).toThrow(
      expect.objectContaining({ code: 'invalid-descriptor', path: ['role'] }),
    );
  });
  ...
  it('returns safe failures for invalid object values', () => {
    const result = safeNormalizeRuntimeSchemaValue(object({ role: string() }), { role: false });

    expect(result).toEqual({
      success: false,
      error: expect.objectContaining({ code: 'invalid-search', path: ['role'] }),
    });
    expect(result.success ? undefined : result.error).toBeInstanceOf(UrlKitError);
  });
});
