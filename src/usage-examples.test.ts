...
import {
  ...
  regexMinUser,
  uuidUser,
} from '../examples/optional-path-params.js';
...
describe('usage examples', () => {
  ...
  it('executes the optional and chained path params example', () => {
    expect(optionalProductWithoutId.params).toEqual({});
    expect(optionalProductWithId.params.id).toBe(2.5);
    expect(optionalProductHrefWithoutId).toBe('/products');
    expect(optionalProductHrefWithId).toBe('/products/2.5');
    expect(regexMinUser.params.id).toBe(2);
    expect(decimalPrice.params.amount).toBe(-9.99);
    expect(uuidUser.params.id).toBe('7d444840-9dc0-11d1-b245-5ffdce74fad2');
    expect(optionalArticleWithoutSlug.params).toEqual({});
    expect(optionalArticleWithSlug.params.slug).toBe('hello-urlkit');
  });
});
